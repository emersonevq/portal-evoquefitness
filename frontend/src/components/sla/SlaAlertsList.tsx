...
export const SlaAlertsList: React.FC<SlaAlertsListProps> = ({
  ...
  return (
    <div className={`sla-alerts-list border rounded-lg overflow-hidden ${getHeaderColor()}`}>
      ...
      {chamados.length === 0 ? (
        <div className="p-4 text-center text-gray-500 text-sm">
          Nenhum chamado {type === 'vencido' ? 'vencido' : type === 'em_risco' ? 'em risco' : 'pausado'}
        </div>
      ) : (
        <div className="divide-y">
          {chamados.slice(0, 10).map((chamado) => (
            <div key={chamado.chamado_id} className="p-3 hover:bg-gray-50 transition">
              <div className="flex items-center justify-between gap-2">
                ...
                <div className="text-right whitespace-nowrap">
                  <span className={`inline-block px-2 py-1 rounded text-xs font-semibold ${getBadgeColor(
